...
static emlrtRTEInfo r_emlrtRTEI = { 77,/* lineNo */
  9,                                   /* colNo */
  "xgeqp3",                            /* fName */
            ...
            tau_size[1], int32_T jpvt_data[], int32_T jpvt_size[2])
{
  ...
  if ((A->size[0] == 0) || (A->size[1] == 0)) {
    ...
    if (b_infocheck(&b_st, (int32_T)info_t)) {
      ...
      emxEnsureCapacity_real_T(&st, A, i27, &r_emlrtRTEI);
      loop_ub = A->size[1];
      for (i27 = 0; i27 < loop_ub; i27++) {
        m = A->size[0];
        for (k = 0; k < m; k++) {
          A->data[k + A->size[0] * i27] = rtNaN;
        }
      }

      for (i27 = 0; i27 < tau_size_tmp; i27++) {
        tau_data[i27] = rtNaN;
      }
      ...
      jpvt_size[1] = m;
      for (i27 = 0; i27 < m; i27++) {
        jpvt_data[i27] = (int32_T)jpvt_t_data[i27];
      }
    }
  }
}
