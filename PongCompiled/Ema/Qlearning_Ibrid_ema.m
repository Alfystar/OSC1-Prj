...
while(i <= 10^6)
    ...
    for j=1:ite
        ...
        yp0 = (H-1)*bRnd+1;
        if(plays<=10)
            [~,Qup,Qdown,Qstill,score,rimbalzi, G] = PongEffectIbrid_mex(xb0,yb0,yp0,Qup,Qdown,Qstill,0,G,1,0);
            alpha = 1;
        else
            [~,Qup,Qdown,Qstill,score,rimbalzi, G] = PongEffectIbrid_mex(xb0,yb0,yp0,Qup,Qdown,Qstill,0,G,0,0);
        end
...
yp0 = (H-1)*bRnd+1;
G=0;
%[chk,Qup,Qdown,Qstill,score,rimbalzi,G] = PongEffectVisual(xb0,yb0,yp0,Qup,Qdown,Qstill,1,G,1,0);
eps = epsOld;
