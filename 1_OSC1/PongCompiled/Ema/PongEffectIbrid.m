...
function [chk,Qup,Qdown,Qstill,score,rimbalzi, G] = PongEffectIbrid(xb0,yb0,ys0,Qup,Qdown,Qstill,~, G, SBROnOff,~)
...
while xb > 0 && counter < maxiter
    ...
    if coin >= eps
        ...
        elseif (SBROnOff == 1 ) % procedo con SBR
            [ctr] = bestControl(scalar2vect(xb,yb,ys,vx,vy), Wdown, Wstill, Wup, 0);
            if ctr == 1 && (ys+Lb <= H-vb)
                ys = ys+vb;
    ...
    else % procedo con SBR
        [ctrN] = bestControl (scalar2vect(xb,yb,ys,vx,vy), Wdown, Wstill, Wup, 0);
...
function ctr = bestControl(state, Wdown, Wstill, Wup, ~)
